...
/**
 * WCCPP_Frontend_Pricing class.
 */
class WCCPP_Frontend_Pricing {
	...
	public static function init() {
		...
		self::init_hooks();
		do_action( 'wc_country_product_price_frontend_princing_init' );
	}
	...
	public static function get_product_property( $value, $product ) {
		if ( in_array( $product->get_type(), array_unique( apply_filters( 'wc_country_product_price_product_types_overriden', array( 'simple', 'variable', 'external', 'course' ) ) ), true ) ) {
	...
	public static function get_price_metadata( $meta_value, $object_id, $meta_key, $single ) {
		...
		if ( $single && in_array( $meta_key, wccpp_get_overwrite_meta_keys(), true ) ) {

			// Remove filter to not going into an endless loop.
			remove_filter( 'get_post_metadata', array( __CLASS__, 'get_price_metadata' ), 10, 4 );

			if ( in_array( $meta_key, wccpp_get_date_on_sale_meta_keys(), true ) && 'manual' === wccpp_the_zone()->get_postmeta( $object_id, '_sale_price_dates' ) ) {

				$meta_value = wccpp_the_zone()->get_postmeta( $object_id, $meta_key );

			} elseif ( in_array( $meta_key, wccpp_get_price_meta_keys(), true ) ) {

				$meta_value = wccpp_the_zone()->get_post_price( $object_id, $meta_key );

			} elseif ( ! in_array( $meta_key, wccpp_get_date_on_sale_meta_keys(), true ) ) {
	...
	public static function get_currency( $currency_code ) {
		return wccpp_the_zone()->get_currency();
	}
	...
	// 	$price_hash[] = wccpp_the_zone()->get_postmetakey() . wccpp_the_zone()->get_currency() . wccpp_the_zone()->get_exchange_rate();
	// 	return $price_hash;
	// }
	...
	public static function set_cart_item_price( $cart_item ) {
		if ( version_compare( WC_VERSION, '3.6', '>=' ) && in_array( $cart_item['data']->get_type(), array_unique( apply_filters( 'wc_country_product_price_product_types_overriden', array( 'simple', 'variable', 'external', 'course' ) ) ), true ) ) {
	...
	// 		$args['meta_key'] = wccpp_the_zone()->get_postmetakey( '_price' ); // WPCS: slow query ok.
	...
	// 	global $wpdb;
	// 	if ( version_compare( WC_VERSION, '3.6', '<' ) ) {
	// 		$args['join'] = str_replace( "meta_key='_price'", "meta_key='" . wccpp_the_zone()->get_postmetakey( '_price' ) . "'", $args['join'] ); // WPCS: slow query ok.
	// 	} else {
	...
	// 		) wcpbc_price ON {$wpdb->posts}.ID = wcpbc_price.post_id", wccpp_the_zone()->get_postmetakey( '_price' ) );
	// 	}
	...
	// 		$meta_query['price_filter']['key'] = wccpp_the_zone()->get_postmetakey( '_price' );
	// 	}
	// 	return $meta_query;
	// }
	...
	// 	return array( wccpp_the_zone()->get_postmetakey( '_price' ) );
	// }
	...
	// 			LEFT JOIN {$wpdb->postmeta} wcpbc_price ON wc_product_meta_lookup.product_id = wcpbc_price.post_id and wcpbc_price.meta_key = '" . wccpp_the_zone()->get_postmetakey( '_price' ) . "'
	// 		";
	...
	// 	$cache_key = 'wcpbc_products_onsale_' . wccpp_the_zone()->get_zone_id();
	...
	// 		GROUP BY post.ID
	// 	", wccpp_the_zone()->get_postmetakey( '_sale_price' ), wccpp_the_zone()->get_postmetakey( '_price' ), $decimals, $decimals ) );
	...
	// 					'exchange_rate' => wccpp_the_zone()->get_exchange_rate(),
	// 					'orig_cost'     => $rate->cost,
	...
	// 			} elseif ( wccpp_the_zone()->get_exchange_rate() !== $rate->wcpbc_data['exchange_rate'] ) {

	// 				$rate->wcpbc_data['exchange_rate'] = wccpp_the_zone()->get_exchange_rate();
	// 				$change = true;
	...
	// 					$rate->cost = wccpp_the_zone()->get_exchange_rate_price( $rate->cost );
	// 				} else {
	// 					$rate->cost = wccpp_the_zone()->get_exchange_rate_price( $rate->cost, false );
	// 				}
	...
	// 				$method->min_amount = wccpp_the_zone()->get_exchange_rate_price( $method->min_amount );
	// 			}
	// 		}
	// 	}
	...
	// 	if ( wccpp_the_zone()->get_disable_tax_adjustment() ) {
	// 		$adjust = false;
	// 	}
	// 	return $adjust;
	// }
	...
	// 		$amount = wccpp_the_zone()->get_exchange_rate_price( $coupon_amount );
	// 		self::set_coupon_prop( $coupon, 'coupon_amount', $amount, $_back );
	// 	}

	// 	if ( $minimum_amount ) {
	// 		$amount = wccpp_the_zone()->get_exchange_rate_price( $minimum_amount );
	// 		self::set_coupon_prop( $coupon, 'minimum_amount', $amount, $_back );

	// 	}
	// 	if ( $maximum_amount ) {
	// 		$amount = wccpp_the_zone()->get_exchange_rate_price( $maximum_amount );
	...
	public static function update_order_meta( $order_id, $data ) {
		// update_post_meta( $order_id, '_wcpbc_base_exchange_rate', wccpp_the_zone()->get_base_currency_amount( 1 ) );
		update_post_meta( $order_id, '_wccpp_pricing_zone', wccpp_the_zone()->get_data() );
	}
}

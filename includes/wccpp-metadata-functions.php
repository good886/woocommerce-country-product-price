<?php
/**
 * WC Country Product Price Metadata Functions.
 *
 * @package WCCPP
 * @version 1.0.0
 */
 ...
 * @return array
 */
function wccpp_get_price_meta_keys() {
	return array_unique( apply_filters( 'wc_country_product_price_price_meta_keys', array( '_price', '_regular_price', '_sale_price' ) ) );
}
 ...
 * @return array
 */
function wccpp_get_date_on_sale_meta_keys() {
	return array_unique( apply_filters( 'wc_country_product_price_date_on_sale_meta_keys', array( '_sale_price_dates_from', '_sale_price_dates_to' ) ) );
}
 ...
 * @return array
 */
function wccpp_get_overwrite_meta_keys() {

	$price_meta_keys = wccpp_get_price_meta_keys();
	$meta_keys       = array_merge( $price_meta_keys, wccpp_get_date_on_sale_meta_keys() );
	...
	return array_unique( apply_filters( 'wc_country_product_price_overwrite_meta_keys', $meta_keys ) );
}
 ...
 * @return array
 */
function wccpp_get_meta_keys_to_delete() {
	$meta_keys = wccpp_get_overwrite_meta_keys();
	array_push( $meta_keys, '_price_method', '_sale_price_dates' );
	return array_unique( apply_filters( 'wc_country_product_price_meta_keys_to_delete', $meta_keys ) );
}
 ...
 * @return array
 */
function wccpp_get_parent_product_types() {
	return array_unique( apply_filters( 'wc_country_product_price_parent_product_types', array( 'variable', 'grouped' ) ) );
}
/**
 ...
 * @return object
 */
function wccpp_get_children_price( $zone_price_meta_key, $parent_id, $min_or_max = 'min' ) {
	global $wpdb;
 ...
 */
function wccpp_zone_variable_product_sync( $zone, $product_id ) {

	foreach ( wccpp_get_price_meta_keys() as $price_metakey ) {
		$zone->delete_postmeta( $product_id, $price_metakey );

		foreach ( array( 'min', 'max' ) as $min_or_max ) {
			$variation_price = '';
			$variation_id    = '';
			$row             = wccpp_get_children_price( $zone->get_postmetakey( $price_metakey ), $product_id, $min_or_max );
			if ( $row ) {
 ...
 */
function wccpp_zone_grouped_product_sync( $zone, $product_id ) {
	$zone->delete_postmeta( $product_id, '_price' );
	foreach ( array( 'min', 'max' ) as $min_or_max ) {
		$row   = wccpp_get_children_price( $zone->get_postmetakey( '_price' ), $product_id, $min_or_max );
 ...
 */
function wccpp_variable_product_sync( $product ) {
	$product_id = is_numeric( $product ) ? $product : $product->get_id();
	foreach ( WCCPP_Pricing_Zones::get_zones() as $zone ) {
		wccpp_zone_variable_product_sync( $zone, $product_id );
	}
}
if ( version_compare( get_option( 'woocommerce_version', null ), '3.0', '<' ) ) {
	add_action( 'woocommerce_variable_product_sync', 'wccpp_variable_product_sync' );
} else {
	add_action( 'woocommerce_variable_product_sync_data', 'wccpp_variable_product_sync' );
}
 ...
 * @param WCCPP_Pricing_Zone $zone Pricing zone instance.
 */
function wccpp_sync_exchange_rate_prices( $zone ) {
	global $wpdb;
	...
	$parent_product_types = wccpp_get_parent_product_types();
	...
	foreach ( wccpp_get_price_meta_keys() as $price_meta_key ) {
	...
	if ( $parent_products ) {
		foreach ( $parent_products as $parent_product ) {
			...
			if ( 'product_variation' === $parent_product->child_post_type ) {
				...
				wccpp_zone_variable_product_sync( $zone, $parent_product->id );

			} else {
				// Sync grouped product price.
				wccpp_zone_grouped_product_sync( $zone, $parent_product->id );
			}
		}
	}
 ...
 * @version 1.8.0
 */
function wccpp_scheduled_sales() {
	global $wpdb;
		...
		foreach ( $parents as $post_type => $parent_ids ) {
			if ( 'product' === $post_type ) {
				...
				foreach ( array_unique( $parent_ids ) as $parent_id ) {
					// Sync grouped product price.
					wccpp_zone_grouped_product_sync( $zone, $parent_id );
				}
				...
				foreach ( array_unique( $parent_ids ) as $parent_id ) {
					...
					// Sync variable product price.
					wccpp_zone_variable_product_sync( $zone, $parent_id );
				}
			}
		}

		// Sync exchange rate prices.
		wccpp_sync_exchange_rate_prices( $zone );
	}

}
add_action( 'woocommerce_scheduled_sales', 'wccpp_scheduled_sales', 20 );
 ...
 */
function wccpp_delete_product_transients( $post_id = 0 ) {

	$transients_to_clear = array(
		'wccpp_products_onsale_',
	);
...
add_action( 'woocommerce_delete_product_transients', 'wccpp_delete_product_transients' );
